...
void setup() {
  ...
  if (config.magic_number != CONFIG_REVISION) {  // this will set it up for very first use
...
void pour_timer() {
  if (millis() > lastPressed + intervalInput)  // 1seconds called
  {
    ...
    if (pourTimer == 1) {  // after increse detected
      second++;
    ...
    if (pourTimer == 2) {  // countdown timer done. prepare next timer val
      // substr += 2;
