...
void startserver() {
  ...
  server.on("/running", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    // request->send(SPIFFS, "/index.html", String(), false, processor);
    request->send(200, "text/plain", (smart_pour) ? "1" : "0");  // hidden setup and pour tab
  });
  ...
  server.on("/changetheme", HTTP_GET, [](AsyncWebServerRequest *request) {
    config.theme = (config.theme == 1) ? 0 : 1;
    request->send_P(200, "text/plain", String(config.theme).c_str());
  });
  ...
  server.on("/netap", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("keyap")) {
      ...
      if (!inputMessage.isEmpty() && inputMessage.length() > 7) {
        ...
        for (size_t i = 0; i < inputMessage.length(); i++) {  // use size_t for length comparison
          EEPROM.write(512 + 32 + i, inputMessage[i]);
        }
        EEPROM.commit();
      } else if (!inputMessage.isEmpty()) {  // added check for empty inputMessage
        config.openAP = true;
        writePref();
      }
    }
